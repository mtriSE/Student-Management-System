...
bool System::getInfo(Course *&course, Student *&student) {
    string student_info;
    string course_info;
...
bool System::getInfoForFile(stringstream &ss_std_cour, Course *&course, Student *&student) {
...
void System::assignForFile(stringstream &Ss)
{
    Course *course;
    Student *student;
...
void System::recordScoreForFile(stringstream &ss_std_cour, stringstream &ss_score)
{
    Course *course;
    Student *student;
    double score;
...
bool System::getCourseInfo(Course *&course)
{
    string course_info;
...
bool System::getStudentInfo(Student *&student)
{
    string student_info;
...
void System::updateGradingFile(string student_id_arg, string course_id_arg, double score_to_add)
{
    ...
    while (!grading_file->eof())
    {
        ...
        if (second_comma != string::npos)
            course_id = line.substr(first_comma + 1, second_comma - first_comma - 1);
        else
            course_id = line.substr(first_comma + 1);

        if (std_id == student_id_arg && course_id_arg == course_id)
        {

            ostringstream os;
            if ((int)score_to_add != score_to_add)
...
void System::loadData()
{
    ...
    while (!student_file->eof())
    {
        ...
        if (line_in_student_file == "")
            break;
        if (line_in_student_file.length() == 1)
            return;
    ...
    while (!course_file->eof())
    {
        ...
        if (line_in_course_file == "")
            break;
        if (line_in_course_file.length() == 1)
            return;
    ...
    while (!grading_file->eof())
    {
        ...
        ss_line_in_grading_file << line_in_grading_file.substr(0, first_comma) + " " + (second_comma != string::npos ? line_in_grading_file.substr(first_comma + 1, second_comma - first_comma - 1) : line_in_grading_file.substr(first_comma + 1));
    ...
    while (!grading_file->eof())
    {
        ...
        if (second_comma == string::npos)
            continue; // that enrollment still havent had score
...
void System::getStudentsInCourse()
{
    Course *course;
...
void System::getCoursesInStudent()
{
    Student *student;
